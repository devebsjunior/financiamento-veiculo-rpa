...
<div class="space-y-4 p-4 max-w-6xl mx-auto w-full">
    ...
    <div class="flex justify-between items-center pb-3 border-b border-slate-200">
        ...
        <div class="flex gap-2">
            ...
            <button onclick="exportarExcel()" class="bg-emerald-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-emerald-700 transition flex items-center gap-1.5 shadow-sm">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                Exportar XLS
            </button>
            <a href="/users/create" class="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-blue-700 transition shadow-sm">
                Novo Usuário
            </a>
        </div>
    </div>
...
<script>
    ...
    // Função sênior para baixar arquivos Blob protegidos por Token JWT
    async function exportarExcel() {
        ...
        try {
            ...
            // Converte a resposta em binário puro (Blob)
            const blob = await response.blob();
            const url = window.URL.createObjectURL(blob);

            // Simula clique invisível para forçar o download no navegador
            ...
            // Limpa lixo da memória ram do navegador
            a.remove();
